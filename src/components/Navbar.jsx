...
const Navbar = () => {
    ...
    const handledashboardclick = async()=>{
      try {
        ...
        console.error("Failed to load to dashboard:", err);
      }
    }
          ...
          <ul className='fixed top-[108px] left-0 w-full h-auto pb-8 border-b bg-white shadow-sm z-50'>
            ...
            <>
              {user.role === 'user' && (
                <li className='flex gap-3 items-center'>
                  <img src={AvaterImg} alt="" className='size-8' />
                  <button onClick={handleLogout} className='bg-[#1E73BE] px-2 py-1.5 text-white rounded-sm'>Logout</button>
                </li>
              )}
              {user.role === 'admin' && (
                <li className='flex gap-3 mt-3 px-4 items-center'>
                  {/* <img src={AvaterImg} alt="" className='size-8' /> */}
                  <Link to="/dashboard" onClick={handledashboardclick}><button className='bg-[#1E73BE] px-4 py-1.5 text-white rounded-sm'>Dashboard</button></Link>
                </li>
              )}
            </>
          ) : (
            <li className='px-4 mt-3 text-black'>
              <NavLink 
                to="/login" 
                onClick={handleloginclick}
                className={({ isActive }) => 
